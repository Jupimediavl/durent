...
exports.autoAcceptExpiredRequests = exports.cancelEndRental = exports.getPastTenants = exports.getPastRentals = exports.getEndRequestStatus = exports.acceptEndRental = exports.requestEndRental = void 0;
const client_1 = require("@prisma/client");
const notificationService_1 = require("../services/notificationService");
const prisma = new client_1.PrismaClient();
// Request to end rental (landlord or tenant)
const requestEndRental = async (req, res) => {
    try {
        ...
        await notificationService_1.NotificationService.sendEndRentalRequestNotification(otherParty.id, requesterName, rental.property.title, rental.propertyId, reason);
        res.status(201).json({
...
exports.requestEndRental = requestEndRental;
// Accept end rental request
const acceptEndRental = async (req, res) => {
    try {
        const { rentalId } = req.params;
        const userId = req.userId;
        ...
        // Auto-set property as AVAILABLE when rental ends
        ...
        await notificationService_1.NotificationService.createAndSendNotification(requester.id, 'END_RENTAL_REQUEST', 'Rental Ended', `The rental for ${rental.property.title} has been ended by mutual agreement.`, { type: 'rental_ended', propertyId: rental.propertyId });
        res.json({
...
exports.acceptEndRental = acceptEndRental;
// Get end request status
const getEndRequestStatus = async (req, res) => {
    try {
        const { rentalId } = req.params;
        const userId = req.userId;
        const rental = await prisma.rentalRelationship.findUnique({
...
exports.getEndRequestStatus = getEndRequestStatus;
// Get past rentals for a user
const getPastRentals = async (req, res) => {
    try {
        const userId = req.userId;
...
exports.getPastRentals = getPastRentals;
// Get past tenants for a property
const getPastTenants = async (req, res) => {
    try {
...
exports.getPastTenants = getPastTenants;
// Cancel end rental request (only by requester)
const cancelEndRental = async (req, res) => {
    try {
        const { rentalId } = req.params;
        const userId = req.userId;
        ...
        const otherParty = userId === rental.tenantId ? rental.landlord : rental.tenant;
        const requesterName = userId === rental.tenantId ? rental.tenant.name : rental.landlord.name;
        await notificationService_1.NotificationService.createAndSendNotification(otherParty.id, 'END_RENTAL_REQUEST', 'End Request Cancelled', `${requesterName} cancelled the end rental request for ${rental.property.title}.`, { type: 'end_rental_cancelled', propertyId: rental.propertyId });
        res.json({
...
exports.cancelEndRental = cancelEndRental;
// Auto-accept expired requests (called by cron job)
const autoAcceptExpiredRequests = async (req, res) => {
    try {
        const now = new Date();
        ...
        for (const request of expiredRequests) {
            ...
            // Auto-set property as AVAILABLE when rental ends
            await prisma.property.update({
                where: { id: request.rental.propertyId },
                data: {
                    isPublic: true,
                    availabilityStatus: 'AVAILABLE'
                }
            ...
            await notificationService_1.NotificationService.createAndSendNotification(tenant.id, 'END_RENTAL_AUTO_ACCEPT_WARNING', 'Rental Ended', `The rental for ${propertyTitle} has been automatically ended after 7 days.`, { type: 'rental_auto_ended', propertyId: request.rental.propertyId });
            await notificationService_1.NotificationService.createAndSendNotification(landlord.id, 'END_RENTAL_AUTO_ACCEPT_WARNING', 'Rental Ended', `The rental for ${propertyTitle} has been automatically ended after 7 days.`, { type: 'rental_auto_ended', propertyId: request.rental.propertyId });
            results.push({
                rentalId: request.rentalId,
...
exports.autoAcceptExpiredRequests = autoAcceptExpiredRequests;
//# sourceMappingURL=endRentalController.js.map