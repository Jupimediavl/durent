...
exports.autoGeneratePayments = autoGeneratePayments;
...
async function generatePaymentForRental(rental, result) {
    ...
    await notificationService_1.NotificationService.sendPaymentReminderNotification(tenant.id, property.title, paymentAmount, nextPaymentDate.toLocaleDateString(), property.id);
    console.log(`✅ Generated payment for rental ${rental.id}: ${paymentAmount} AED due ${nextPaymentDate.toLocaleDateString()}`);
    result.success++;
}
function calculateNextPaymentDate(today, dueDay, frequency) {
...
function endOfPeriod(date, frequency) {
    if (frequency === 12) {
...
//# sourceMappingURL=autoGeneratePayments.js.map