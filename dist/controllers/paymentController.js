...
const markPaymentAsPaid = async (req, res) => {
    try {
        ...
        if (userId === payment.rental.tenantId) {
            await notificationService_1.NotificationService.createAndSendNotification(payment.rental.landlordId, 'PAYMENT_VERIFICATION_NEEDED', 'Payment Verification Needed', `${payment.rental.tenant.name} submitted payment proof for ${payment.rental.property.title}`, {
...
const verifyPayment = async (req, res) => {
    try {
        ...
        if (verify) {
            await notificationService_1.NotificationService.createAndSendNotification(payment.rental.tenantId, 'PAYMENT_APPROVED', 'Payment Approved', `Your payment of AED ${payment.amount} for ${payment.rental.property.title} has been approved`, {
                paymentId: payment.id,
                propertyId: payment.rental.propertyId,
                propertyTitle: payment.rental.property.title,
                amount: payment.amount
            ...
            await notificationService_1.NotificationService.createAndSendNotification(payment.rental.tenantId, 'PAYMENT_REJECTED', 'Payment Rejected', `Your payment of AED ${payment.amount} for ${payment.rental.property.title} was rejected. Please resubmit.`, {
                paymentId: payment.id,
                propertyId: payment.rental.propertyId,
                propertyTitle: payment.rental.property.title,
                amount: payment.amount,
